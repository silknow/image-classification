...
def make_label_statistics(class_label_list, output_path, output_file,
                          label):
    full_fig_name = os.path.join(output_path, output_file + '.jpg')
...
#    plt.rcParams["figure.figsize"] = [8, 8]
    ...
    if result_folder is not None:
        fig_id.savefig(os.path.join(result_folder, title + '.jpg')) 
...
def add_jpeg_decoding(input_height, input_width, input_depth, module_spec,
                      bool_hub_module,
    ...
    aspect_ratio = tf.maximum(decoded_height/decoded_width,
